...
# ---------------------------------------------------------------- 先手敏捷表
# 來源：彩虹城 Starryyy 製作「先手敏捷表」（2023 年 2 月版本，使用者提供截圖）。
# 數值＝使用黑暗儀式後、比該 BOSS 先出手所需的最低敏捷。
# 同倍數數值必相同（截圖已逐格辨識並以此交叉驗證）。
FS_BOSSES = ["威1", "黑白", "古堡B1", "威彩", "夢奇100區", "鬼3",
             "龍1", "龍2", "龍3", "寧靜凍原", "冰洞B1/B2"]
FS_MULT_ROWS = {
    "2":    [74, 79, 67, 86, 87, 105, 105, 107, 123, 156, 153],
    "1.5":  [98, 104, 88, 114, 115, 139, 140, 142, 163, 208, 204],
    "1.4":  [105, 112, 95, 122, 123, 149, 150, 152, 175, 222, 218],
    "1.35": [108, 116, 98, 127, 128, 154, 155, 158, 181, 231, 226],
    "1.3":  [113, 120, 102, 132, 133, 160, 161, 164, 188, 239, 235],
    "1.25": [117, 125, 106, 137, 138, 167, 167, 171, 195, 249, 244],
    "1.2":  [122, 130, 110, 143, 144, 174, 174, 178, 204, 259, 254],
    "1.15": [127, 136, 115, 149, 150, 181, 182, 185, 212, 271, 265],
    "1.05": [139, 149, 126, 163, 164, 198, 199, 203, 232, 296, 291],
    "1":    [146, 156, 132, 171, 172, 208, 209, 213, 244, 311, 305],
    "0.9":  [163, 173, 147, 191, 192, 232, 233, 238, 272, 347, 340],
    "0.75": [194, 208, 176, 228, 229, 277, 278, 284, 325, 414, 406],
    "0.7":  [208, 222, 188, 244, 245, 297, 298, 304, 348, 444, 435],
}
# (職業, [(技能, 圖示編號, 出手順序, 倍數), ...])——職業名依典藏版慣例（原圖用大劍師/格鬥家/暗術師/祭司）
FS_JOBS = [
    ("大劍士", [("地裂閃", 1, 6, "1.4"), ("天翔閃", 2, 7, "1.35"),
                ("星爆龍翔閃", 3, 8, "1.3")]),
    ("刀狂", [("海洋斬", 4, 10, "1.2"), ("天空斬", 5, 13, "1.05"),
              ("鳳翼斬", 6, 14, "1")]),
    ("狂戰士", [("驟雨斧", 7, 11, "1.15"), ("急電斧", 8, 9, "1.25"),
                ("獸王九擊", 9, 10, "1.2")]),
    ("巫師", [("召喚武神", 10, 14, "1"), ("召喚火神", 11, 11, "1.15"),
              ("血液蒸發", 12, 10, "1.2"), ("流星爆擊", 13, 20, "0.7")]),
    ("光術師", [("聖靈祝福", 14, 8, "1.3"), ("恩惠之雨", 15, 10, "1.2")]),
    ("闇術師", [("闇影遁身", 16, 16, "0.9"), ("生命儀式", 17, 20, "0.7"),
                ("黑洞爆發", 18, 20, "0.7")]),
    ("祭師", [("金光魔刀", 19, 8, "1.3"), ("巨木連擊", 20, 9, "1.25"),
              ("自然之力", 21, 9, "1.25"), ("炙焰刀", 22, 4, "1.5"),
              ("火焰衝擊", 23, 11, "1.15"), ("土遁術", 24, 2, "2")]),
    ("大老闆", [("普卡攻擊", 25, 9, "1.25"), ("金卡攻擊", 26, 9, "1.25"),
                ("白金卡攻擊", 27, 11, "1.15")]),
    ("格鬥師", [("裂空拳", 28, 10, "1.2"), ("滅空拳", 29, 10, "1.2"),
                ("幻空拳", 30, 11, "1.15")]),
    ("魔獸使", [("強化", 31, 9, "1.25"), ("合擊", 32, 11, "1.15"),
                ("合體", 33, 19, "0.75"), ("獸神衝擊", 34, 20, "0.7")]),
]


def build_firststrike():
    jobs = []
    for job, skills in FS_JOBS:
        jobs.append({"j": job, "s": [
            {"n": n, "i": i, "o": o, "k": k, "v": FS_MULT_ROWS[k]}
            for n, i, o, k in skills]})
    return {"bosses": FS_BOSSES, "jobs": jobs,
            "src": "彩虹城 Starryyy 製作・2023 年 2 月版本"}
